...
def _render_detail(title: str, description: str, path: str, body_html: str, json_ld: dict) -> str:
    ...
    return render_template(
        ...
        active_section=_publication_section(path),
        content_kind=_publication_kind(path),
    )
...
def _publication_section(path: str) -> str:
    if path.startswith("bulletin"):
        return "bulletins"
    if path.startswith("sitrep"):
        return "sitreps"
    if path.startswith("countr"):
        return "countries"
    return ""


def _publication_kind(path: str) -> str:
    if path.startswith("bulletin"):
        return "Weekly bulletin"
    if path.startswith("sitrep"):
        return "Situation report"
    if path.startswith("countr"):
        return "Country intelligence"
    return "Humanitarian intelligence"


def _text_excerpt(value: str, limit: int = 220) -> str:
...
def _sitrep_sources(report: dict) -> list[dict]:
    sources = report.get("narrative_sources") or {}
    ...
    for key, value in sources.items():
        ...
        if not re.fullmatch(r"https?://[^\s]+", url):
            continue
        title = str(value.get("title") or f"Source {key}").strip()
        items.append({"index": int(key), "title": title, "url": url})
...
def _sitrep_title(report: dict, filename: str) -> str:
    ...
    raw = re.sub(r"[_-]+", " ", raw)
    raw = re.sub(r"\breport\b", "", raw, flags=re.IGNORECASE)
    tokens = []
...
def _link_sitrep_citations(narrative: str, sources: list[dict]) -> str:
    available = {item["index"] for item in sources}
    sanitized = _sanitize_html(narrative)
    ...
    items = [
        {
            ...
            "subtitle": f"{b.get('week_start', '')} — {b.get('week_end', '')}",
            "summary": (
                f"A sourced weekly briefing covering "
                f"{_count_label(b.get('countries_affected', 0), 'country', 'countries')} "
                f"and {_count_label(b.get('crises_count', 0), 'priority crisis', 'priority crises')}."
            ),
            "metrics": [
                _count_label(b.get("total_reports", 0), "report"),
                _count_label(b.get("countries_affected", 0), "country", "countries"),
            ],
        }
        for b in list_bulletins()
    ]
...
@seo_bp.route("/sitreps")
def sitrep_list():
    """Public archive of generated situation reports and their sources."""
    record_page_view("/sitreps", request.headers.get("User-Agent", ""))
    items = []
    for filename, slug in _sitrep_report_files():
        try:
            with open(OUTPUT_REPORTS_DIR / filename, encoding="utf-8") as fh:
                report = json.load(fh)
        except Exception:
            continue
        sources = _sitrep_sources(report)
        themes = report.get("themes") or []
        items.append(
            {
                "url": f"/sitrep/{slug}",
                "title": _sitrep_title(report, filename),
                "eyebrow": "Situation report",
                "subtitle": _lastmod(OUTPUT_REPORTS_DIR / filename),
                "summary": _text_excerpt(report.get("summary") or "A sourced humanitarian situation report."),
                "metrics": [_count_label(len(sources), "source"), _count_label(len(themes), "theme")],
            }
        )
    return _render_list(
        "Humanitarian Situation Reports",
        "Evidence-led SITREPs with traceable citations, operational findings and direct links to source material.",
        items,
        "sitreps",
    )
...
def bulletin_detail(slug: str):
    ...
    def _render() -> str:
        ...
        if trimmed.get("global_overview"):
            sections.append(
                '<section class="bulletin-overview"><p class="section-label">Weekly analysis</p>'
                f"<h2>Global overview</h2><p>{_sanitize_html(trimmed['global_overview'])}</p></section>"
            )
        crises = trimmed.get("crises", [])
        if crises:
            crisis_items = []
...
def sitrep_detail(slug: str):
    ...
    title = _sitrep_title(report, filename)
    sources = _sitrep_sources(report)
    narrative_raw = report.get("narrative_html") or report.get("narrative") or ""
    narrative = _link_sitrep_citations(narrative_raw, sources)
    if not narrative and report.get("summary"):
        narrative = f"<h2>Executive overview</h2><p>{_sanitize_html(report['summary'])}</p>"
    sections = narrative if narrative else "<p>No narrative available.</p>"
    if sources:
        source_items = "".join(
            (
                f'<li id="source-{source["index"]}"><span>{source["index"]:02d}</span>'
                f'<a href="{html.escape(source["url"], quote=True)}" target="_blank" '
                f'rel="noopener noreferrer">{html.escape(source["title"])}</a></li>'
            )
    ...
    json_ld = {
        ...
        "publisher": {"@type": "Organization", "name": "Sightline"},
        "image": f"{SITE_URL}/static/logo-signal-horizon.png",
        "citation": [source["url"] for source in sources],
    }
