...
class TestProposalsAPI:
    ...
    def test_create_and_get_proposal_flow(self, client):
        ...
        with (
            ...
            get_resp = client.get(f"/api/proposals/{prop_id}", headers={"Authorization": "Bearer token"})
            ...
            update_resp = client.put(
                ...
                headers={"Authorization": "Bearer token"},
                json={"title": "WASH South Sudan v2", "donor": "USAID"},
            )
            assert update_resp.status_code == 200

            # Verify update
            get_resp2 = client.get(f"/api/proposals/{prop_id}", headers={"Authorization": "Bearer token"})
            get_data2 = json.loads(get_resp2.data)
            assert get_data2["title"] == "WASH South Sudan v2"
            assert get_data2["donor"] == "USAID"

            # 4. List proposals
            list_resp = client.get("/api/proposals", headers={"Authorization": "Bearer token"})
            ...
            # 5. Delete proposal
            delete_resp = client.delete(f"/api/proposals/{prop_id}", headers={"Authorization": "Bearer token"})
    ...
    def test_llm_generation_flows(self, client):
        ...
        with (
            patch.object(auth, "_dev_mode", return_value=False),
            patch.object(auth, "verify_firebase_token", return_value=fake_token),
            patch("reliefweb_api.vector_store.VectorStore"),
        ):
            ...
            with patch("sitrep.llm_client.chat", return_value=mock_toc_response):
                toc_resp = client.post(
                    f"/api/proposals/{prop_id}/generate-toc", headers={"Authorization": "Bearer token"}
                )
            ...
            with patch("sitrep.llm_client.chat", return_value=mock_lf_response):
                lf_resp = client.post(
                    f"/api/proposals/{prop_id}/generate-logframe", headers={"Authorization": "Bearer token"}
                )
            ...
            with patch("sitrep.llm_client.chat", return_value=mock_narrative_response):
                narr_resp = client.post(
                    f"/api/proposals/{prop_id}/generate-narrative", headers={"Authorization": "Bearer token"}
                )
    ...
    def test_proposal_chunks_endpoint(self, client):
        ...
        with (
            ...
            mock_chunks = [
                {
                    "text": "Water points destroyed in Darfur",
                    "title": "Sudan sitrep",
                    "date": "2026-04-10",
                    "themes": "WASH",
                }
            ...
            with patch("sitrep.chroma_adapter.ChromaAdapter") as mock_adapter_class:
                ...
                chunks_resp = client.get(f"/api/proposals/{prop_id}/chunks", headers={"Authorization": "Bearer token"})
    ...
    def test_proposal_advisor_agent_chat(self, client):
        ...
        with (
            ...
            hist_resp = client.get(
                f"/api/proposals/{prop_id}/advisor/history", headers={"Authorization": "Bearer token"}
            )
    ...
    def test_proposal_export_endpoint(self, client):
        ...
        with (
            ...
            exp_resp = client.post(f"/api/proposals/{prop_id}/export", headers={"Authorization": "Bearer token"})
